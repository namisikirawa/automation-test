...
namespace ClinicApp_Test.Form
{
    public class PatientForm
    {
        ...
        public DataGridView PatientGrid => _window.FindFirstDescendant(cf => cf.ByAutomationId("dgv_Patient"))?.AsDataGridView();
        private AutomationElement PatientManagement => _window.FindFirstDescendant(cf => cf.ByAutomationId("PatientManagement"));

        private AutomationElement GroupBox => PatientManagement?.FindFirstDescendant(cf => cf.ByAutomationId("groupBox1"));

        public TextBox NameTextBox => GroupBox?.FindFirstDescendant(cf => cf.ByAutomationId("txtHoTen"))?.AsTextBox();

        public TextBox PhoneTextBox => GroupBox?.FindFirstDescendant(cf => cf.ByAutomationId("txtSdt"))?.AsTextBox();

        public Button SearchButton => GroupBox?.FindFirstDescendant(cf => cf.ByAutomationId("btnSearch"))?.AsButton();
        ...
        //nhấn nút sửa bệnh nhân đầu tiên
        public void EditFirstPatient(UIA3Automation automation, int processId)
        {
            var grid = PatientGrid;
            if (grid == null)
                throw new Exception("Không tìm thấy bảng bệnh nhân!");
