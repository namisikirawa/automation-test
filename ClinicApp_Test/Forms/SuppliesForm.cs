...
namespace ClinicApp_Test.Form
{
    public class SuppliesForm
    {
        ...
        public DataGridView SuppliesGrid => _window.FindFirstDescendant(cf => cf.ByAutomationId("dgv_Supplies"))?.AsDataGridView();
        private AutomationElement GroupBox => _window.FindFirstDescendant(cf => cf.ByAutomationId("groupBox1"));

        private TextBox TxtTenvattu => GroupBox.FindFirstDescendant(cf => cf.ByAutomationId("txtTenvattu"))?.AsTextBox();

        private TextBox TxtNhacungcap => GroupBox.FindFirstDescendant(cf => cf.ByAutomationId("txtNhacungcap"))?.AsTextBox();
        ...
        // ---tìm vật tư theo tên hoặc nhà cung cấp ---
        ...
        //nhấn nút sửa vật tư đầu tiên
        public void EditFirstSupplies(UIA3Automation automation, int processId)
        {
            var grid = SuppliesGrid;
            if (grid == null)
                throw new Exception("Không tìm thấy bảng vật tư!");
